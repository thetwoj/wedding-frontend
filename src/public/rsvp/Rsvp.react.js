  ...
  submitRsvp = (event) => {
    ...
    for (let i = 0; i < invitation.guests.length; i++) {
      ...
      if (guest.attending && !(guest.slider1 && guest.slider2)) {
        this.setState({error: 'Please select two sliders for each attending guest'});
        return;
      }
    }
    ...
    invitation.guests.forEach((guest) => {
      fetch('/api/rsvps/' + this.state.invitation.id + '/guests/' + guest.id, {
        ...
        .then(resp => {
          if (!resp.ok) {
            this.setState({error: 'RSVP could not be submitted, please try again'});
            throw new Error();
          }
      ...
      guests.forEach((guest, index) => {
        guestEdits.push(
          <Paper className="rsvpGuest" key={'guestEdit' + index}>
            ...
            {!guest.invited_by &&
            <p className={"guestName"}><b>{guest.name}</b></p>
            }
            ...
            <SelectField
              ...
              onChange={(event, key, payload) => this.handleAttendingChange(event, guest, payload)}
              fullWidth={true}>
            ...
            <p><b><a href={""} onClick={this.toggleSliderDetails}>Slider menu</a></b></p>

            <SelectField
              disabled={!guest.attending}
              floatingLabelText={'Choose first slider'}
              value={guest.slider1}
              onChange={(event, _, item) => this.handleSlider1Change(guest, item)}
              fullWidth={true}>
            ...
            <SelectField
              ...
              value={guest.slider2}
              onChange={(event, _, item) => this.handleSlider2Change(guest, item)}
              fullWidth={true}>
              {this.sliderMenuItems()}
            </SelectField>
            <br/>
      ...
      <div>
        ...
        <div className={"centered-content-block"}>
          <span className={"centered-content"}>
            Check out the <a href={"/#logistics"}>wedding logistics</a> and <a href={"/#faq"}>FAQ</a> before submitting your RSVP
          </span>
        </div>
        }

        {(!this.state.invitation && !this.state.showThanks) &&
          ...
          <div className="accessCodeContainer">
            <div className="accessCode">
              <TextField
                ...
                fullWidth={true}
              />
              <RaisedButton className="getRsvpSubmit" label="Submit" onClick={this.getRsvp}/>
            </div>
          </div>
        </form>
        }
        ...
        <form onSubmit={this.submitRsvp}>
          ...
          <div className={"rsvpGuestContainer"}>
            <RaisedButton className="rsvpSubmit" label="Submit" onClick={this.submitRsvp}/>
          </div>
        </form>
        }
        ...
        >
          <br/>
          <Paper zDepth={1}>
            <div className={"sliderName sliderText"}><b>Burlington</b></div>
            <img className="slider" src={burlington} alt={''}/>
            <div className={"sliderText"}>{this.state.sliders ? this.state.sliders[2].description : ''}</div>
          </Paper>
          <br/>

          <Paper zDepth={1}>
            <div className={"sliderName sliderText"}><b>Hawthorne</b></div>
            <img className="slider" src={hawthorne} alt={''}/>
            <div className={"sliderText"}>{this.state.sliders ? this.state.sliders[1].description : ''}</div>
          </Paper>
          <br/>

          <Paper zDepth={1}>
            <div className={"sliderName sliderText"}><b>Ross Island</b></div>
            <img className="slider" src={rossisland} alt={''}/>
            <div className={"sliderText"}>{this.state.sliders ? this.state.sliders[3].description : ''}</div>
          </Paper>
          <br/>

          <Paper zDepth={1}>
            <div className={"sliderName sliderText"}><b>Sellwood</b></div>
            <img className="slider" src={sellwood} alt={''}/>
            <div className={"sliderText"}>{this.state.sliders ? this.state.sliders[0].description : ''}</div>
          </Paper>
          <br/>

          <Paper zDepth={1}>
            <div className={"sliderName sliderText"}><b>Tilikum</b></div>
            <img className="slider" src={tilikum} alt={''}/>
            <div className={"sliderText"}>{this.state.sliders ? this.state.sliders[4].description : ''}</div>
          </Paper>
          <br/>
          For additional details please visit the <a href="http://pdxsliders.com">PDX Sliders website</a>
        </Dialog>
      </div>
    )
  }
