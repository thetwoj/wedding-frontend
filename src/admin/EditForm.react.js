...
class EditForm extends React.Component {
  ...
  handleGuestDelete(event, guest, index) {
    if (!guest.id) {
      ...
      fetch(
        "/api/invitations/" + guest.invitation + "/guests/" + guest.id + "/",
        {
          method: "delete",
          headers: {
            Accept: "application/json",
  ...
  render() {
    ...
    const invitationEdit = (
      <Paper className="editInvitation">
        ...
        <FlatButton
          label="Delete"
          onClick={event => this.handleInvitationDelete(event)}
        />
      </Paper>
    );
      ...
      guests.forEach((guest, index) => {
        guestEdits.push(
          <Paper className="editGuest" key={"guestEdit" + index}>
            ...
            <SelectField
              name="attending"
              value={guest.attending === null ? "no_response" : guest.attending}
              floatingLabelText="Attending"
              onChange={(event, key, payload) =>
                this.handleDropdownChange(index, payload)
              }
              fullWidth={true}
            >
            ...
            <FlatButton
              label="Delete"
              onClick={event => this.handleGuestDelete(event, guest, index)}
            />
          </Paper>
        );
      });
    ...
    if (this.state.invitation.id) {
      addGuest = (
        <FlatButton
          label="Add guest"
          onClick={event => this.props.createGuest(event)}
        />
      );
    }
